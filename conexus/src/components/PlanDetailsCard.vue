        ...
        <v-card-item>
            <div>
                ...
                <div class="d-flex flex-row align-center mt-8">
                    <div :class="isActualPlan ? 'text-h6 font-weight-bold text-custom-actual me-1' : 'text-h6 font-weight-bold text-custom me-1'">R$ {{ planPrice }}</div>
            ...
            <div class="mt-8 d-flex align-center" v-if="isAdditional">
                <v-icon class="me-2" color="orange">mdi-check-circle</v-icon>
                <div class="font-weight-bold text-subtitle-2 text-custom-check">Adicional do setup contratado</div>
            </div>
            <div v-else class="font-weight-bold text-center text-custom-check text-subtitle-2 mt-8" style="width: 80%; margin: auto;">Setup inicial personalizado está disponível à parte por R$ {{ setupPrice }}</div>
